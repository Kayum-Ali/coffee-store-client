...
const CoffeeCard = ({ coffee, coffeed, setCoffeed }) => {
  ...
  return (
    <div className="bg-[#F5F4F1] flex flex-col lg:flex-row p-8 gap-8 lg:gap-20 items-center lg:justify-around rounded-md">
      <div>
        <img src={coffee?.photo} alt={coffee?.name} className="w-full" />
      </div>
      <div>
        <h2 className="text-2xl text-black font-medium">
          Name : <span className="raleway text-[#A8A7A6] text-xl">{coffee?.name}</span>
        </h2>
        <h2 className="text-2xl text-black font-medium">
          Chef : <span className="raleway text-[#A8A7A6] text-xl">{coffee?.chef}</span>
        </h2>
        <h2 className="text-2xl text-black font-medium">
          Taste :{" "}
          <span className="raleway text-[#A8A7A6] text-xl">{coffee?.taste}</span>
        </h2>
      </div>
      <div className="flex lg:flex-col flex-row raleway gap-5">
       ...
       <Link to={`/update-coffee/${coffee?._id}`}>
        ...
        <button onClick={() => handleDelete(coffee?._id)} className="btn btn-error">
          {" "}
          Delete
        </button>
      </div>
